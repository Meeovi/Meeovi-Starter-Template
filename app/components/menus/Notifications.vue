<template>
  <div>
    <v-btn class="relative" icon="fas fa-bell" variant="text" @click="drawer = !drawer"
      aria-label="Notifications">
    </v-btn>
    <v-badge v-if="unreadCount > 0" :content="unreadCount" color="error" overlap></v-badge>
    
       <!-- Flyout Menu -->
    <v-navigation-drawer v-model="drawer" :location="location" temporary class="cart-flyout">
...
<script setup>
  ...
  import {
    useNotifications
  } from '@meeovi/layer-shared/dist/app/composables/globals/useNotifications'

  const drawer = ref(null)
  const location = ref('right')
  const {
    notifications,
    unreadCount,
    markAsRead
